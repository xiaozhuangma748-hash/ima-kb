...
| 类别 | 功能 | 入口 |
|---|---|---|
| **入库** | 11 种格式解析 + OCR 降级 + SHA256 去重 + LLM 自动标签 | `ima ingest` / `/ingest` / `/note` `/clip` `/url` |
...
├── run.py                        # CLI 入口（21 个顶层命令 + graph 5 子命令）
├── repl.py                       # 交互式 REPL（v4.0 Claude Code 风格，30+ 子命令）
...
│   │   ├── reader.py             #   智能阅读器
│   │   └── comparator.py         #   文档对比
│   ├── agent/agent.py            # LLM Agent（ReAct 工具调用）
...
│  入口层（Entry Layer）                                        │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │  CLI (run.py) │  │ REPL(repl.py)│  │ Web (web/)   │       │
│  │  21 顶层命令  │  │  30+ 子命令   │  │ 7 页面+API   │       │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘       │
└─────────┼─────────────────┼─────────────────┼───────────────┘
          │                 │                 │
          └─────────────────┼─────────────────┘
...
#### 文件：[repl.py](file:///Users/4u/Desktop/项目/拱墅区/2025身后事（殡葬）项目/34-知识库/repl.py)

项目最核心的交互入口（约 4300 行），实现 Claude Code 风格的终端常驻对话界面。

##### 核心特性

- **欢迎面板**：左窄右宽布局（左 32 列：mascot+宠物信息+状态 / 右：Tips+Recent activity），中间 `│` dim 竖线分隔，顶部标题线 `── IMA v4.0 ──`
...
- `PIXEL_PET_ASCII` — 启动页像素风吉祥物（8 行 15 列，已缩小）
...
##### 文件：[core/agent/agent.py](file:///Users/4u/Desktop/项目/拱墅区/2025身后事（殡葬）项目/34-知识库/core/agent/agent.py)
...
#### 文件：[web/static/app.js](file:///Users/4u/Desktop/项目/拱墅区/2025身后事（殡葬）项目/34-知识库/web/static/app.js)

##### 关键函数

| 函数 | 说明 |
|---|---|
| `sendMessage()` | SSE 流式接收 + Markdown 渲染 + 引用编号可点击 |
| `handleFiles(files)` | FormData 上传到 `/api/ingest/upload` |
| `doSearch()` | 调 `/api/search`，关键词高亮（`<mark>` 包裹） |
| `renderAnalyze(data)` | 渲染 sheet tabs + 字段统计 + 预览表格 + AI 解读 |
| `loadDashboard()` | 调 `/api/stats` 更新指标 + 标签柱状图 + 告警 |
| `loadGraph()` | vis.Network 渲染图谱，节点点击调邻居 API |
| `loadPet()` | 宠物状态加载与互动 |
...
| 能力 | CLI (run.py) | REPL (repl.py) | Web (web/) |
|---|---|---|---|
