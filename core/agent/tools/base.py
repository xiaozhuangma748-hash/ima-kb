...
_PROMPT_HEADER = (
    ...
    '{"tool": "工具名", "args": "参数"}\n'
    "\n"
    "最终答案格式（重要！用 ANSWER: 标记，不要用 JSON）：\n"
    "ANSWER:你的最终答案文本\n"
    "\n"
    "可用工具列表：\n"
)
...
_PROMPT_DONE_BLOCK = (
    'done — 任务完成，给出最终答案\n'
    '   格式：ANSWER:最终答案文本（注意：ANSWER: 后直接跟答案，不要换行，不要加引号）'
)
...
_PROMPT_FOOTER = (
    ...
    "ANSWER:你好！我是知识库管理助手，可以帮你查询和检索知识库中的文档。\n"
    "\n"
    "Thought: 用户问我的身份，我简要回答。\n"
    "ANSWER:我是知识库管理助手，专门帮你从知识库中检索信息。\n"
    "\n"
    "绝对禁止：直接输出答案而不带 Thought 前缀。\n"
    "\n"
    "# 最终答案格式（再次强调！）\n"
    "\n"
    "最终答案必须用 ANSWER: 标记，不要用 JSON 的 done 工具：\n"
    "- ✅ 正确：ANSWER:答案是...\n"
    '- ❌ 错误：{"tool": "done", "args": "答案是..."}\n'
    "\n"
    "ANSWER: 后直接跟答案文本，不要换行、不要加引号、不要包裹 JSON。\n"
    "答案可以包含换行（在 ANSWER: 之后的文本都视为答案内容）。\n"
    "\n"
    "# 引用规范（重要！）\n"
    "\n"
