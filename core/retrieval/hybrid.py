...
# - precise（精确查询，含具体名称/编号）→ BM25 与向量均衡（0.5/0.5，避免向量被完全压制）
# - fuzzy（口语化/语义查询，含疑问词）→ 偏向向量（语义相似度更重要）
# - normal（其他）→ 略偏向向量
QUERY_WEIGHTS = {
    # precise 由 0.7/0.3 调整为 0.5/0.5：原权重下向量第 1 名（0.3/31≈0.0097）
    # 连 BM25 第 5 名（0.7/35≈0.02）都打不过，导致开启向量后排序完全被 BM25 压制，
    # 智能混合/混合检索与关键词检索结果几乎一致。均衡后向量语义召回有机会进入 top-K。
    "precise": {"bm25": 0.5, "vector": 0.5},
    "fuzzy":   {"bm25": 0.3, "vector": 0.7},
    "normal":  {"bm25": 0.4, "vector": 0.6},
}
