...
class RAGChain:
    ...
    def ask(
        ...
        # 7. 构造引用列表（只保留 LLM 实际引用的文档）
        cited_indices = set(re.findall(r'\[(\d+)\]', content))
        citations = [
            {
                "index": i + 1,
    ...
    def ask_stream(
        ...
        cited_indices = set(re.findall(r'\[(\d+)\]', answer_text))
        cited_results = [r for i, r in enumerate(final_results) if str(i + 1) in cited_indices]
        if cited_results:
            yield "\n\n" + "=" * 50 + "\n"
            yield "📚 引用来源：\n"
