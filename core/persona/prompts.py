...
## 引用规则
- 引用标记 [n] 一一对应上方"检索到的资料"中的第 [n] 块（[1] 对应 [1]，[2] 对应 [2]，以此类推）
- 关键事实必须引用，常识无需引用
- 多个资料支撑同一观点时合并引用 [1][3]
- 禁止使用超出资料块数量的编号（如资料只有 3 块，就不能用 [4]）
"""
...
def build_system_prompt(
    ...
    prompt += "- 禁止在回答中标注 [偏好]、[身份]、[事实]、[主题] 等记忆类型标记，这些是内部系统标签，用户不需要看到\n"
